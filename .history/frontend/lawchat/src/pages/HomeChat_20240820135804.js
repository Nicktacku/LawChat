...
const HomeChat = () => {
  ...
  const handleGcDelete = async (gcId) => {
    await axios
      .delete(`http://localhost:5555/gc/delete/${gcId}`)
      .then((response) => {
        console.log(response);
      })
      .catch(function (error) {
  ...
  const handleNewNameModal = () => {
    setNewNameModal(!newNameModal);
  };

  const handleGroupRename = async (e) => {
    e.preventDefault();
    await axios
      .put(`http://localhost:5555/gc/rename/${currentGc}`, { name: groupName })
      .then((response) => {
        console.log(response);
      })
      .catch(function (error) {
  ...
  return (
    <div className="container">
      ...
      <div className="chats">
        ...
        <img src={newchaticon} alt="New Chat Icon" onClick={handleModal} />
        <div
          id="myModal"
          class="modal"
        ...
        {gcs.map((gc) => (
          <>
            ...
            >
              <ChatPreview key={gc._id} name={gc.name} preview={gc.last_chat} />
            </div>
            <button onClick={() => handleGcDelete(gc._id)}>delete</button>
          </>
        ))}
      </div>
      <div className="chat">
        <div className="chat-bar">
          ...
          <img
            ...
            className="chat-menu"
            alt="Menu Icon"
            onClick={handleNewNameModal}
          />
          <div
            id="myModal"
            class="modal"
            style={newNameModal ? { display: "block" } : { display: "none" }}
          >
            <div class="modal-content">
              <span class="close" onClick={handleNewNameModal}>
                &times;
              </span>
              <h2>Rename Group Chat</h2>
              <form>
                ...
                <input
                  ...
                  value={groupName}
                />
                <button onClick={handleGroupRename}>rename</button>
              </form>
            </div>
          </div>
        </div>
